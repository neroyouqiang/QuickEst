...
def test_models(models, X, Y, silence=False):
    ...
    scores_norm2 = np.zeros([len(models)], dtype=np.float)
    ...
    for ii in xrange(len(models)):
        ...
        scores_norm2[ii] = np.sqrt(1 - metrics.r2_score(Y[:, ii], results[:, ii]))
    
    # print scores
    if not silence: print "RAE of the results are: ", scores_RAE
    if not silence: print "R2 of the results are: ", scores_R2
    if not silence: print "Norm-2 metric of the results are: ", scores_norm2
    
    # return 
    return results


def test_models2(models, X, Y, silence=False):
    # test model
    results = np.zeros([X.shape[0], len(models)], dtype=np.float)
    scores_RAE = np.zeros([len(models)], dtype=np.float)
    scores_R2 = np.zeros([len(models)], dtype=np.float)
    ...
    for ii in xrange(len(models)):
        # load models
        model_xgb_fsel = models[ii][0]
        sel_features = models[ii][1]
        model_xgb_bagging = models[ii][2]
        model_lasso_bagging = models[ii][3]
        
        # predict
        results0 = predict_fsel(X, model_xgb_fsel, sel_features)
        results1 = predict_bagging(X, model_xgb_bagging)
        results2 = predict_bagging(X, model_lasso_bagging)
        results[:, ii] = (results0 + results1 + results2) / 2.0
        # results[:, ii] = results2
...
def predict_fsel(X, model, sel_features):
    return model.predict(X[:, sel_features])


def predict_bagging(X, models):
    _predict = np.zeros([X.shape[0], len(models)])
    for ii in xrange(len(models)):
        _predict[:, ii] = models[ii].predict(X)
